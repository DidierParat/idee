package idee.resource;

import idee.client.ClientException;
...
public class GetIdeasResource {
  ...
  public String getIdeas(
    ...
    if (areasNearby.isEmpty()) {
      log.info("No area found nearby the provided location.");
    ...
    for (final Area area : areasNearby) {
      try {
        ...
        log.warn("Could not get forecast for area " + area.getId(), exception);
        return createJsonResponse(Collections.emptySet());
      }
    }
    if (forecastNearbyAreas.isEmpty()) {
      log.info("No area with weather forecast corresponding to user's preferences.");
      return createJsonResponse(Collections.emptySet());
    }
    ...
    for (final Forecast forecast : forecastNearbyAreas) {
      ...
      try {
        ...
        log.warn("Could not get trips for area " + areaId, exception);
        continue;
      }
    ...
    if (ideas.isEmpty()) {
      log.info("No Idea found.");
      return createJsonResponse(Collections.emptySet());
    }

    log.info("I found " + ideas.size() + " Ideas!");
